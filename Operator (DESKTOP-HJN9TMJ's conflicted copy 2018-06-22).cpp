...
bool BestInsertPlace(int request_index,const Problem& p, Solution& S, double cost, std::vector<bool>& bookkeep )
{
    ...
    {
                 for(auto singletrip_it=vehicle_trip_it->Multi.begin();
                    singletrip_it!=vehicle_trip_it->Multi.end();singletrip_it++)
                {
                    // tripnum++;
                    // cout<<" tripnum== "<<tripnum;
                    SingleTrip& tempStrip=S.GlobalTrips[*singletrip_it];
                    
                    if(!tempStrip.islunchtrip)
                    {
                        cout<<"1";
