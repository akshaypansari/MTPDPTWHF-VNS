...
bool LocalSearch::LocalSearch_operator2(Solution& solution,const Problem& p)//a customer relocation
{
   ...
   int request_index=solution.unrouted_cust_request_id[rand()%solution.unrouted_cust_request_id.size()];//contain index of request in request
    ...
    {
        solution.MTrips[iter->vehicletrip_id].Multi.erase(std::remove(solution.MTrips[iter->vehicletrip_id].Multi.begin(), solution.MTrips[iter->vehicletrip_id].Multi.end(),RandomTripNumber), solution.MTrips[iter->vehicletrip_id].Multi.end());
...
{
    m_LS2_bookkeep.resize(p.num_of_request);//maybe a local variable
    for(unsigned i=0;i<p.num_of_request;i++)
    {
        m_LS2_bookkeep[i]=0;
    }
    int total_local_search_operators=20;
